...
describe("UI Store", () => {
    ...
    it.each([
        true,
        false,
    ])("should change menu status on event for %s", (initial) => {
        const bus = spiedBus();
    ...
    it("Should clear a form", () => {
        ...
        expect(state.forms[CREATE_COLLECTION_FORM]).toEqual({
            error: null,
            values: {},
        });
        expectRender(bus);
...
function aManagerWith(state: State, bus: Nanobus) {
